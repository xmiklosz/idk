...
int init_taskMgr() {
    ...
    task_mgr = (tTaskMgr *)(ram_base + task_mgr_frame_id * ram->page_size);
...
int create_task(const tPageTableEntry *page_table, uint8_t max_frames, void *address_space) {
    ...
    // CRITICAL FIX: Copy page table entry by entry, preserving ALL fields including frame_id
    // Then only modify p_bit, r_bit, m_bit
    for (int i = 0; i < PAGE_TABLE_SIZE; i++) {
        // Copy the entire entry (this includes r, w, x, AND frame_id)
        task_mgr->tasks[free_slot].page_table[i] = page_table[i];

        // Now only clear the state bits (not present initially)
        task_mgr->tasks[free_slot].page_table[i].p_bit = 0;
        task_mgr->tasks[free_slot].page_table[i].r_bit = 0;
        task_mgr->tasks[free_slot].page_table[i].m_bit = 0;
        // frame_id, r, w, x are preserved from page_table[i]
    }
