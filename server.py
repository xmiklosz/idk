...
class SensorServer:
    ...
    async def send_binary(self, addr, msg_dict):
        binary_data = encode_message(msg_dict)
        try:
    ...
    async def handle_message(self, data, addr):
        try:
            ...
            safe_print("Nie je možné dekódovať binárnu správu:", e)
            return
        ...
        if mtype == "ping":
            resp = {"type": "ping_response", "timestamp": now_unix(), "token": token, "device_type": device_type}
            await self.send_binary(addr, resp)
            return
