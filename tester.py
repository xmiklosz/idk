...
from binary_protocol import (
    encode_message, decode_message, verify_checksum_binary,
    DEVICE_MAP
)
        ...
        for attempt in range(1, max_retries + 1):
            try:
                ...
                if corrupt_first and attempt == 1:
                    # Corrupt the checksum by modifying the last byte
                    binary_data = binary_data[:-1] + bytes([(binary_data[-1] + 1) & 0xFF])
                sock.sendto(binary_data, addr)
            ...
            if not resp.get("checksum_valid", False):
...
def send_and_wait_until_ack(sock, addr, obj, stop_event=None, timeout=TIMEOUT, retry_interval=0.2):
    ...
    while True:
        if stop_event and stop_event.is_set():
            return None
        try:
            binary_data = encode_message(base_obj)
            try:
                sock.sendto(binary_data, addr)
            except Exception as e:
...
def ping_listener(listen_sock, sensor_name, token, ping_counter, resume_event, stop_event):
    ...
    try:
        while not stop_event.is_set() and not resume_event.is_set():
            ...
            if not msg.get("checksum_valid", False):
                continue
    ...
    while True:
        ...
        if menu_choice == "1":
            ...
            for key, (sensor_name, sensor_func) in SENSOR_FUNCS.items():
                ...
                token = ""
                if resp and resp.get("checksum_valid", False):
