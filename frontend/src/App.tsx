...
import { useWallet, CONTRACT_ADDRESS, CHAIN_ID } from "./hooks/useContract";
    ...
    <div className="min-h-screen flex flex-col">
      <header className="border-b border-slate-800 bg-slate-900/80 backdrop-blur sticky top-0 z-10">
        <div className="max-w-5xl mx-auto px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <Link to="/" className="font-bold text-lg">CommitMarket</Link>
      ...
      <footer className="border-t border-slate-800 mt-10">
        <div className="max-w-5xl mx-auto px-4 py-4 text-xs text-slate-500 flex flex-wrap gap-3 justify-between">
          <span>DMBLOCK Assignment 2 — Commit-Reveal Prediction Market</span>
          <span className="font-mono">
            chain {CHAIN_ID} · {CONTRACT_ADDRESS.slice(0, 6)}…{CONTRACT_ADDRESS.slice(-4)}
          </span>
        </div>
      </footer>
    </div>
